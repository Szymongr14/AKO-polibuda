...
string_to_detect db 'elo'
string_to_detect_length equ $ - string_to_detect
...
.code
	...
	mov esi, 0
	mov edi, 0
	mov ebx, 0 ;zerowanie zmiennej ktora przechowuje informacje o wystapieniu okreslonego ciagu znakow
	

	take_next_character:
		cmp ebx, string_to_detect_length ;sprawdzenie czy indeks ebx jest na ostatnim znaku szukanego stringa
		jne not_detected ;jesli nie to nic nie rob
		sub edi, string_to_detect_length ;jesli tak to odejmij od edi dlugosc szukanego ciagu znakow
		mov ax, 2615h ;wstaw wartosc ktora chcesz ustawic
		mov ebx, 0 ;wyzeruj licznik 
		jmp convert_to_utf_16 ;przejdz to wpisania znaku do buffor_out
		...
		inc esi
		cmp al, string_to_detect[ebx] ;sprawdzenie czy pobrany znak jest stringu do wykrycia pod indeksem ebx
		jne wrong_character
		inc ebx ;zwiekszenie idkesu string_to_detect
		
		check_normal_letters:
		cmp al, 'a'
	...
	;etykieta potrzebna do zerowania indeksu znaleziania znaku w szukanum ciagu
	wrong_character:
	mov ebx, 0
	mov ah, 0
	jmp check_normal_letters
END